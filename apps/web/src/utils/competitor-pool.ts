import { hasCompetitorPriceActivity } from "@amazon-monitor/shared";
import type {
  AsinWatchLevel,
  AsinWatchState,
...
export function filterVisibleCompetitors(
  options: FilterVisibleCompetitorsOptions,
): CompetitorPoolItem[] {
  ...
  return options.competitors.filter((item) => {
    const matchesSource =
      ...
      (options.competitorSourceFilter === "category" &&
        item.sourceType === "hybrid") ||
      (options.competitorSourceFilter === "keyword" &&
        item.sourceType === "hybrid");
    const matchesTier = matchesCompetitorTierFilter(
      item,
      options.competitorTierFilter,
      watchByAsin,
    );
    ...
    return (
      ...
      (item.asin.toLowerCase().includes(query) ||
        ...
        item.competitorReasons.some((reason) =>
...
export function findSelectedCompetitor(
  competitors: CompetitorPoolItem[],
  selectedCompetitorAsin: string | null,
): CompetitorPoolItem | null {
  return (
    competitors.find((item) => item.asin === selectedCompetitorAsin) ?? null
  );
}
...
export type KpiDelta = CompetitorDailyKpiDelta;
...
function hasPriceActivity(item: CompetitorPoolItem): boolean {
  return hasCompetitorPriceActivity(item);
}
...
export function buildCompetitorKpis(
  ...
  const coreCount = competitors.filter((item) =>
    isEffectiveCoreCompetitor(item, watchByAsin),
  ).length;
  const newCount = competitors.filter((item) => isNewEntry(item, today)).length;
  const priceActiveCount = competitors.filter((item) =>
    hasPriceActivity(item),
  ).length;
  ...
  return [
    ...
    },
    {
      ...
      label: "新进竞品",
      value: newCount,
      delta: yesterdayDelta.new,
      tone: "new",
    },
    {
      key: "priceActive",
      label: "价格波动",
      value: priceActiveCount,
      delta: yesterdayDelta.priceActive,
      tone: "price",
    },
    {
      key: "key",
...
export function buildCompetitorInsightSuggestion(
  ...
  const promoActive = competitors.filter((item) =>
    Boolean(validCouponText(item.couponText)),
  );
  const coreItems = competitors.filter((item) =>
    isEffectiveCoreCompetitor(item, watchByAsin),
  );

  // 头部建议:找出"价格活跃 + 核心/观察分层"重合度最高的 ASIN
  const priceActiveCore = priceActive.filter((item) =>
    isEffectiveCoreCompetitor(item, watchByAsin),
  );
  const headline =
    priceActiveCore.length > 0
      ? "建议优先关注:价格波动 + 促销中 + 核心竞品"
      : "建议优先关注:核心竞品中带价格活动的 ASIN";
  const body =
    priceActiveCore.length > 0
...
function watchStateByAsin(
  watchStates: AsinWatchState[],
): Map<string, AsinWatchState> {
  return new Map(watchStates.map((state) => [state.asin, state]));
}

function watchLevelFor(
...
function isEffectiveCoreCompetitor(
  item: CompetitorPoolItem,
  watchByAsin: Map<string, AsinWatchState>,
): boolean {
  return (
    ...
    watchLevelFor(item, watchByAsin) === "CORE"
  );
}
...
function matchesCompetitorTierFilter(
  ...
  if (filter === "watch")
    return (
      item.competitorTier === "watch" ||
      watchLevelFor(item, watchByAsin) !== "NORMAL"
    );
