...
export function extractProductDetailRanks(): {
  ...
  function findDealBadge(): string | null {
    return findPromoText(
      [
        ...
        "#priceblock_dealprice",
        "#dealsAccordionRow",
        "#primeExclusivePricingMessage_feature_div",
        "#centerCol",
        "#rightCol",
        "#buybox",
        ".dealBadge",
  ...
  function findPromoText(selectors: string[], patterns: RegExp[], excludePattern?: RegExp): string | null {
    ...
    for (const candidate of promoCandidates) {
      if (excludePattern && excludePattern.test(candidate)) {
        continue;
      }
