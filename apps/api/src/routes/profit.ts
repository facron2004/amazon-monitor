...
import type { ProfitPlanLevel, SessionContext } from "@amazon-monitor/shared";
import { z } from "zod";
...
function requireOperatorOrAdmin(ctx: SessionContext): void {
  if (ctx.user.role !== "admin" && ctx.user.role !== "operator") {
    throw Object.assign(new Error("Forbidden: only operator or admin can manage profit settings"), { statusCode: 403 });
  }
}
...
export function registerProfitRoutes(app: Express, store: Store): void {
  app.get("/api/profit/plans", asyncHandler(async (request, response) => {
    ...
    response.json(store.listProfitPlans({
