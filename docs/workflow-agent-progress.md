...
- Replaced optional strict-output fields with required nullable scope/evidence fields and fixed per-action payload schemas. A packaged SDK smoke run crossed schema serialization and reached OpenAI authentication, removing the pre-network failure that previously made every real-key run impossible.
- Hardened utility-process recovery: Main retains the safeStorage Key only in memory and re-injects it when a replacement Agent reports ready; an Agent crash fails active runs without replay, while API startup reconciles interrupted runs, steps, and tool calls and preserves collection-waiting recovery runs.
...
- Final repository gates passed with 901 tests (`59` shared, `16` Agent, `600` API, `219` Web, `7` Desktop), the root production build, and 9 real-browser tests. The root test command now includes the Desktop workspace. Agent V1.0 is versioned as `1.0.0`; the current unsigned NSIS installer is 331,304,874 bytes with SHA-256 `9CC495259F20E44703B485982502453EFF2222708C6756763356FB6E96AF55D1`.
- A real silent upgrade pass installed `0.6.1`, upgraded the same directory to `1.0.0`, then uninstalled it. The formal userData database retained the same 68,911,104-byte size and SHA-256 `A9225FD1944ADA020CE2937281257C7845A754C5D20122E2BF11E85CB8D2FF9B`; integrity and key business counts remained unchanged after uninstall.
