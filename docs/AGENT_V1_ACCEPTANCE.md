...
| 里程碑 | 状态 | 当前证据 |
|---|---|---|
| M0 真实数据底座 | 通过 | 根级测试覆盖五个 workspace 并以 913 个测试正常退出；根级生产构建与浏览器测试通过；SQLite、压缩包、`tmp/`、评审暂存目录和测试日志未进入提交。v0.7/SP-API、安全底座与 Agent 源码已形成提交 `37ba0ce`。 |
...
| M3 单 Agent 与 API | 通过（OAuth 只读实测）/待完整场景验收 | 单 Orchestrator、SQLite Session、10 turns、流式事件、主模型重试/备用模型、工具瞬态重试、SSE 重连、取消、审计、关联 recovery。桌面端现支持 OpenAI、OpenAI-compatible 与 ChatGPT OAuth 多连接自由切换；OAuth 通过内置 Codex app-server 和 15 个动态业务工具运行，API Key 路径支持 Responses / Chat Completions。最终 EXE 已完成 OAuth 只读查询并返回结构化结果；30 题运行器已实测顺序执行、超时自动取消和单题失败继续，ASIN、审批、补采恢复及指标达标仍待验收。 |
...
| M6 灰度与发布 | 待指标达标与人工标注 | `AGENT_SDK_ENABLED` 默认关闭；自动更新默认关闭；多模型接入已同步为 SemVer `1.1.0` 并生成 NSIS。OAuth 账号查询、受限线程配置、动态工具 Schema、EXE 保存/切换和一条只读结构化对话均已实测；30 题报告已生成，但本次 60 秒/题边界下指标仍未达标，且高优提醒/恢复标注未完成。 |
...
| 6 | 模型/Agent/Crawler 崩溃后 UI 可用且不重放写操作 | 通过（进程/持久化）/待真实模型中断 | 强杀 API 后 Renderer 存活且 API 以新 PID 恢复；强杀 Agent 与 Crawler 后三进程恢复 running，重启 Agent 重新获得 safeStorage Key。Agent 退出会终止 active runs，API 启动会把中断运行、步骤和工具调用标记 failed 并记录 `replayed: false`，等待采集的 recovery 保持可恢复；取消后的迟到桌面事件不会重新激活终态运行。真实模型流中断仍待 Key。 |
...
- 单元测试：913（shared 59、Agent 18、API 602、Web 219、Desktop 15）；Desktop 已纳入根级 `npm test`。
- 浏览器测试：9。
- 根级生产构建：通过。
...
- 最新 NSIS：
  ...
  - 大小：428,026,383 bytes
  - SHA-256：`1637D0A70B0ED72814C3F476AB3171E7A698947BFDBB1B463963A3A8BA4C72D2`
...
1. 本机已在可见验收实例内完成 ChatGPT OAuth；其他验收环境仍需完成 OAuth 或通过 Windows safeStorage 保存真实供应商 API Key，且不得把凭据写入对话、终端、SQLite 或日志。
2. 在最终安装包内运行核心场景 1–6，导出审计 JSON。
3. 用正式评测超时窗口（默认 600 秒/题）复跑 30 题真实金标集并记录五项指标；本轮 60 秒/题结果仅作为稳定性与取消链路证据，不能替代指标达标验收。运行器通过安装包 API 调用 safeStorage 中的 Key，自动发现当前组织的真实类目、关键词、ASIN 与品牌，逐题保存审计并要求人工标注提醒有效性和恢复结果。指标不达标则继续修正，不以评分器单测代替。
